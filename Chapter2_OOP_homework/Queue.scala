...
trait Doubling extends ArrayBuffer[Int]{
  def doubling(): Unit = this.mapInPlace( _ * 2 ) // 原地修改数组，元素乘以2
}
trait Incrementing extends ArrayBuffer[Int]{
  def increase():Unit = this.mapInPlace( _ + 1 ) // 原地修改数组，递增1
}
trait FilterNegative extends ArrayBuffer[Int]{
  def filterNegative():Unit = this.filterInPlace( _  >= 0 ) // 原地过滤数组，保留非负数
}

class MyQueue extends Doubling with Incrementing with FilterNegative{
  def put(i:Int) = this.addOne(i) // 实现队列入队，直接增加
...
object MyQueue{
  def main(args: Array[String]): Unit = {
    ...
    myQueue.addAll(Array(-5,-4,-3,-3,-2,-1,0,1,2,3,4,5))
